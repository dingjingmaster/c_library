...
XdndWorkaround::XdndWorkaround()
{
    ...
    if(err == nullptr) {
        mXinput2Enabled = true;
        mXinputOpCode = reply->major_opcode;
        mXinputEventBase = reply->first_event;
        mXinputErrorBase = reply->first_error;
        // qDebug() << "xinput: " << m_xi2Enabled << m_xiOpCode << m_xiEventBase;
    } else {
        mXinput2Enabled = false;
        free(err);
    }
    free(reply);
}
...
bool XdndWorkaround::genericEvent(xcb_ge_generic_event_t* event)
{
    ...
    if(mXinput2Enabled && event->extension == mXinputOpCode)
    {
...
{
    QDragManager* mgr = QDragManager::self();
    mLastDrag = mgr->object();
    // qDebug() << "BUTTON RELEASE!!!!" << xcbDrag()->canDrop() << mLastDrag;
}
