/*************************************************************************
> FileName: gio-application-base.c
> Author  : DingJing
> Mail    : dingjing@live.cn
> Created Time: 2021年01月18日 星期一 15时14分42秒
 ************************************************************************/
#include <gio/gio.h>
#include <string.h>
...
static void action1_activated (GSimpleAction *action, GVariant* parameter, gpointer user_data)
{
    g_print ("activate action1\n");
}

static void action2_activated (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    GVariant *state;
...
static void change_action2 (GSimpleAction *action, GVariant *state, gpointer user_data)
{
...
static int command_line (GApplication* application, GApplicationCommandLine *cmdline)
{
    ...
    if (argc > 1) {
        if (g_strcmp0 (argv[1], "echo") == 0) {
            ...
            g_object_get (cmdline, "is-remote", &remote, NULL);
...
static gboolean action_cb (gpointer data)
{
    gchar **argv = data;
    GApplication *app;
    gchar **actions;
    ...
    if (g_strcmp0 (argv[1], "./actions") == 0) {
        ...
        if (g_strcmp0 (argv[2], "list") == 0) {
            ...
            g_action_group_change_action_state (G_ACTION_GROUP (app), "action2", g_variant_new_boolean (TRUE));
        }
