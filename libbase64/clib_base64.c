...
size_t _base64_encode(char* target, const void* source, size_t bytes, const char* table) {
    ...
    for(i = j =  0; i < bytes / 3 * 3; i += 3) {
        ...
        target[j++] = table[((ptr[i] & 0x0F) << 2)\
                | ((ptr[i + 1] >> 6) & 0x03)];                  /* 取第二个 8bit 的后四位 + 第三个 8bit 的前 两位 */
        target[j++] = table[(ptr[i] & 0x3F)];                   /* 取第三个 8bit 的后六位 */
    }
